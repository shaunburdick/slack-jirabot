...
Bot.prototype.slackOpen = function() {
  ...
  logger.verbose("Welcome to Slack. You are @%s of %s", this.slack.self.name, this.slack.team.name);
  logger.verbose('You are in: %s', channels.join(', '));
  logger.verbose('As well as: %s', groups.join(', '));
  messages = unreads === 1 ? 'message' : 'messages';
  return logger.verbose("You have %d unread %s", unreads, messages);
};
