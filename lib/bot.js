...
class Bot {
  ...
  formatIssueDescription(description) {
    ...
    return desc
      ...
      // Bold (not needed as it's the same)
      // .replace(/\*(\S.*)\*/g, '*$1*')

      // Italic (not needed as it's the same)
      ...
      // Superscript (ignored as unsupported)
      // .replace(/\^([^^]*)\^/g, '<sup>$1</sup>')

      // Subscript (not a real format, but I need to change format to prevent Strikethrough)
      .replace(/~([^~]*)~/g, '~~$1~~')
